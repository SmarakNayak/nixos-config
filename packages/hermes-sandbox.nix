...
# Sandboxed Hermes Agent using bubblewrap
#
# --unshare-all        isolates all namespaces (pid, ipc, uts, mount, user, cgroup)
# --share-net          re-enables network so Hermes can reach AI APIs and git remotes
...
# --bind ~/.hermes     punch through Hermes state and config for persistence
# --bind ~/hermes-workspace  dedicated read-write workspace for long-running Hermes work
...
# --bind $SSH_AUTH_SOCK  ssh agent socket for git push/pull over ssh urls; does not expose private keys
# --chdir ~/hermes-workspace  starts Hermes in the dedicated workspace instead of arbitrary $PWD
pkgs.writeShellScriptBin "hermes-sandbox" ''
  mkdir -p "$HOME/.hermes"
  ...
  exec ${pkgs.bubblewrap}/bin/bwrap \
    ...
    --bind "$HOME/hermes-workspace" "$HOME/hermes-workspace" \
    ...
    --chdir "$HOME/hermes-workspace" \
    -- ${pkgs.llm-agents.hermes-agent}/bin/hermes "$@"
''
